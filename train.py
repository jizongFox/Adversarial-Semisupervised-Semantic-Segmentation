...
def parse_args():
    ...
    parser.add_argument("--init_net", choices=('imagenet', 'mscoco'), default='imagnet',
...
def snapshot(model, valoader, epoch, best_miou, snapshot_dir, prefix, args):
    miou = val(model, valoader, nogpu=args.nogpu)
    snapshot = {
        'epoch': epoch,
...
def train_base(generator, optimG, trainloader, valoader, args):
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        best_miou = snapshot(generator, valoader, epoch, best_miou, args.snapshot_dir, args.prefix, args)
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        for batch_id, (img, mask, ohmask) in enumerate(trainloader):
            ...
            print("[{}][{}] LD: {:.4f} LDfake: {:.4f} LD_real: {:.4f} LG: {:.4f} LG_ce: {:.4f} LG_adv: {:.4f}" \
                  .format(epoch, itr, (LDr + LDf).data[0], LDr.data[0], LDf.data[0], LGseg.data[0], LGce.data[0],
                          LGadv.data[0]))
        best_miou = snapshot(generator, valoader, epoch, best_miou, args.snapshot_dir, args.prefix, args)
...
def train_semi(generator, discriminator, optimG, optimD, trainloader_l, trainloader_u, valoader, args):
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        while (True):
            ...
            print(
                "[{}][{}] LD: {:.4f} LD_fake: {:.4f} LD_real: {:.4f} LG: {:.4f} LG_ce: {:.4f} LG_adv: {:.4f} LG_semi: {:.4f}" \
...
def main():
    ...
    trainset_l = PascalVOC(home_dir, args.dataset_dir, img_transform=Compose(imgtr), label_transform=Compose(labtr),
                           co_transform=Compose(cotr), split=args.split, labeled=True)
    trainset_u = PascalVOC(home_dir, args.dataset_dir, img_transform=Compose(imgtr), label_transform=Compose(labtr),
                           co_transform=Compose(cotr), split=args.split, labeled=False)
    ...
    if args.mode != "base":
        ...
        if args.d_optim == 'adam':
            optimD = optim.Adam(filter(lambda p: p.requires_grad, discriminator.parameters()), lr=args.d_lr,
                                weight_decay=0.0001)
        else:
            optimD = optim.SGD(filter(lambda p: p.requires_grad, \
                                      discriminator.parameters()), lr=args.d_lr, weight_decay=0.0001, momentum=0.5,
                               nesterov=True)
